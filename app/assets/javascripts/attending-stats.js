...
var ageData = [$('.attending_data_age_count').data('temp')];
ageName =[];
ageFreq = [];
...
var attending_data = $('.attending_data_submission_dates').data('temp');
...
var svg = d3.select("#attending-submissions-graph").append("svg")
    .attr("width", width + margin.left + margin.right)
    .attr("height", height + margin.top + margin.bottom)
  .append("g")
    .attr("transform", "translate(" + margin.left + "," + margin.top + ")");

attending_data.forEach(function(d) {
  d.date = parseDate(d.date);
  d.close = +d.close;
});

x.domain(d3.extent(attending_data, function(d) { return d.date; }));
y.domain(d3.extent(attending_data, function(d) { return d.close; }));
...
svg.append("path")
    .datum(attending_data)
    .attr("class", "line")
    .attr("d", line);

var genderValues = [$('.attending_data_gender_count').data('temp')][0];
var genderGraph = new HorizontalBarGraph('#attending-gender-graph', [
...
var ageValues = [$('.attending_data_age_count').data('temp')][0];
var ageGraph = new HorizontalBarGraph('#attending-age-graph', [ageValues]);
...
// $statsPink: #FF8099;
// $statsPink2: #FF5476;
var gradeValues = [$('.attending_data_grade_count').data('temp')][0];
// First Year, Second Year, Third Year, Fourth Year, Fifth Year, Graduate Student, Not a Student, High School Student
var gradeGraph = new HorizontalBarGraph('#attending-grade-graph', [
...
var hackathonValues = [$('.attending_data_hackathon_count').data('temp')][0];
// First Year, Second Year, Third Year, Fourth Year, Fifth Year, Graduate Student, Not a Student
var hackathonGraph = new HorizontalBarGraph('#attending-hackathon-graph', [
...
var tshirtValues = [$('.attending_data_tshirt_count').data('temp')][0];
// First Year, Second Year, Third Year, Fourth Year, Fifth Year, Graduate Student, Not a Student
var tshirtGraph = new HorizontalBarGraph('#attending-tshirt-graph', [
...
var data_common_words = [$('.attending_data_common_words').data('temp')][0].slice(12,82);
...
if (data_common_words.length == 0) {
  ...
  $("#attending-wordcloud").jQCloud(word_array, {
